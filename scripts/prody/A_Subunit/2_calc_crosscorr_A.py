#!/usr/bin/env python3
"""
ProDy ANM cross-correlation calculation for Calcineurin A subunit.
...
Method:
    ...
    - ProDy calcCrossCorr() for the residue cross-correlation matrix

This script ONLY generates the ANM cross-correlation matrices.
Residue mapping, DeltaCC, and top/bottom-20 calculations are separate steps.
"""
...
def analyze_structure(pdbfile, label, chain, cutoff, gamma, n_modes, outfile):
    ...
    # Build ANM model explicitly with fixed parameters.
    anm = ANM(f"{label}_ANM")
    anm.buildHessian(ca, cutoff=cutoff, gamma=gamma)

    # Calculate exactly the requested number of non-zero modes.
    ...
    # Cross-correlation using the calculated ANM modes.
    ...
    # Save residue order so the matrix rows/columns are fully traceable.
...
def main():
    ap = argparse.ArgumentParser(
        description="Calculate ProDy ANM cross-correlation matrices for A subunit."
    )
    ap.add_argument(
        "--bos",
    ...
    ap.add_argument(
        ...
        help="6TZ6 A-subunit PDB (default: 6TZ6_A_Subunit.pdb)",
    )
    ap.add_argument(
        "--chain",
        default="A",
        help="Protein chain ID to analyze (default: A)",
    )
    ap.add_argument(
        "--cutoff",
        type=float,
        default=15.0,
        help="ANM cutoff in Angstrom (default: 15.0)",
    )
    ap.add_argument(
        "--gamma",
        type=float,
        default=1.0,
        help="ANM gamma value (default: 1.0)",
    )
    ap.add_argument(
        "--modes",
        type=int,
        default=20,
        help="Number of non-zero ANM modes (default: 20)",
    )
    ...
    analyze_structure(
        ...
        gamma=args.gamma,
        n_modes=args.modes,
        outfile="1TCO_A_crosscorr.csv",
    )

    analyze_structure(
        pdbfile=args.candida,
        label="6TZ6_A",
        chain=args.chain,
        cutoff=args.cutoff,
        gamma=args.gamma,
        n_modes=args.modes,
        outfile="6TZ6_A_crosscorr.csv",
    )
    ...
    print("=" * 72)
    print("Created:")
    print("  1TCO_A_crosscorr.csv")
    print("  6TZ6_A_crosscorr.csv")
    print("  1TCO_A_residues.csv")
    print("  6TZ6_A_residues.csv")
