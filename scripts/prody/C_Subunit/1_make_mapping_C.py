#!/usr/bin/env python3
"""
Create residue mapping for FKBP12 / C subunit (1TCO C vs 6TZ6 C).
...
Outputs:
    mapping_C_new.csv
    C_sequence_alignment.txt

Mapping CSV format:
    Bos residue,Residue (C),Candida residue,Residue (C)

This script performs sequence/residue mapping only; it does NOT run ANM.
"""
...
def read_ca_residues(pdb_file, chain="C"):
            ...
            mapping.append(
                {
                    ...
                    "Residue (C)": pretty_three_letter(b["resname3"]),
                    "Candida residue": c["resnum"],
                    "Residue (C)_Candida": pretty_three_letter(c["resname3"]),
                }
            )
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate C-subunit residue mapping for 1TCO vs 6TZ6."
    )
    ap.add_argument(
        "--bos",
    ...
    ap.add_argument(
        "--chain",
        default="C",
        help="Protein chain ID to map (default: C)",
    )
    ap.add_argument(
        "--out",
        default="mapping_C_new.csv",
        help="Output mapping CSV (default: mapping_C_new.csv)",
    )
    ap.add_argument(
        "--alignment-out",
        default="C_sequence_alignment.txt",
        help="Text alignment output (default: C_sequence_alignment.txt)",
    )
    ap.add_argument(
        "--old-mapping",
        default="mapping_C.csv",
        help="Existing mapping used only for QC (default: mapping_C.csv)",
    )
    args = ap.parse_args()
    ...
    print(f"1TCO C C-alpha residues : {len(bos)}")
    print(f"6TZ6 C C-alpha residues : {len(candida)}")
    ...
    with open(args.alignment_out, "w", encoding="utf-8") as f:
        f.write("1TCO C vs 6TZ6 C global sequence alignment\n")
    ...
    if mapping:
        ...
        print(
            "First mapping pair     : "
            f"{first['Bos residue']} {first['Residue (C)']} -> "
            f"{first['Candida residue']} {first['Residue (C)_Candida']}"
        )
        print(
            "Last mapping pair      : "
            f"{last['Bos residue']} {last['Residue (C)']} -> "
            f"{last['Candida residue']} {last['Residue (C)_Candida']}"
        )
    ...
    print("\nIMPORTANT:")
    print("  Review mapping_C_new.csv before using it downstream.")
