...
# 6TZ6 FK506: one-PDB -> topology -> single-snapshot MM/GBSA pipeline
# Usage:
#   bash run_analysis_6TZ6_from_PDB.sh [input.pdb] [prefix]
# Example:
#   bash run_analysis_6TZ6_from_PDB.sh 6TZ6_FK506_MASTER_MAESTRO_PREP.pdb 6TZ6_FK506

INPUT_PDB="${1:-6TZ6_FK506_MASTER_prep.pdb}"
PREFIX="${2:-6TZ6_FK506}"
LIGAND="FK5"
LIGAND_CHARGE=0
VDW_LIMIT=100000
...
if lig_h_count != 69:
    raise SystemExit(f'ERROR: Nötr FK506 için 69 hidrojen bekleniyordu; {lig_h_count} bulundu. '
                     'Maestro-prep edilmiş, tüm hidrojenleri içeren PDB kullanılmalı.')
...
if len(prot) != 4954 or len(lig) != 57:
    raise SystemExit(f'ERROR: Ağır atom sayıları beklenenden farklı: protein={len(prot)} (4954), FK5={len(lig)} (57).')
...
print(f'PASS: 618 protein kalıntısı, 3 doğru TER kesintisi, ağır atom sayıları, {lig_h_count} FK5 hidrojeni ve <1 Å clash kontrolü.')
PY
